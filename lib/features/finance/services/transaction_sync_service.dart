...
class TransactionSyncService implements ISyncService<Transaction> {
  final TransactionStorage _localStorage;
  final FirestoreTransactionStorage _cloudStorage;
  final ErrorHandler _errorHandler;
  final Future<bool> Function() _isCloudSyncEnabled;
  final LoggerService _logger = LoggerService();

  late final GenericSyncQueue<Transaction> _syncQueue;
  ...
  TransactionSyncService({
    ...
    _syncQueue = GenericSyncQueue<Transaction>(
  ...
  @override
  Future<void> syncToCloudDebounced(Transaction item, String userId) async {
  ...
  Future<void> _syncSingleItem(Transaction item, String userId) async {
  ...
  Future<SyncOperationResult> syncFromCloud(
    ...
    try {
      ...
      for (final cloudItem in cloudItems) {
        await _mergeCloudItem(cloudItem);
      }
  ...
  Future<void> _mergeCloudItem(Transaction cloudItem) async {
    final localItem = await _localStorage.getByKey(
      cloudItem.id,
    ); // Assuming ID is unique enough
    if (localItem == null) {
  ...
  @override
  Future<void> addToQueue(Transaction item, String userId) async {
  ...
  @override
  Future<void> moveToDeadLetter(SyncQueueItem<Transaction> item) async {}
  @override
  Future<List<SyncQueueItem<Transaction>>> getDeadLetterItems() async => [];
  @override
  Future<SyncOperationResult> retryDeadLetterItem(
    SyncQueueItem<Transaction> item,
