...
class FirestoreTaskFinanceLinkStorage
    implements ICloudStorageWithTimeout<TaskFinanceLink> {
  static const String collectionName = 'finance_task_links';
  final ErrorHandler _errorHandler;
  final LoggerService _logger = LoggerService();

  FirestoreTaskFinanceLinkStorage(this._errorHandler);
  ...
  @override
  Future<CloudOperationResult<TaskFinanceLink>> create(
    TaskFinanceLink item,
    String userId,
  ) async {
  ...
  @override
  Future<CloudOperationResult<TaskFinanceLink>> createWithTimeout(
    TaskFinanceLink item,
    String userId,
    Duration timeout,
  ) async {
    try {
      ...
      _logger.debug(
        'Finance',
        '[FirestoreTaskFinanceLinkStorage] Created: ${item.id}',
      );
  ...
  Future<CloudOperationResult<void>> updateWithTimeout(
    ...
    try {
      ...
      _logger.debug(
        'Finance',
        '[FirestoreTaskFinanceLinkStorage] Updated: $documentId',
      );
  ...
  Future<CloudOperationResult<void>> delete(
    ...
    try {
      ...
      _logger.debug(
        'Finance',
        '[FirestoreTaskFinanceLinkStorage] Deleted: $documentId',
      );
      return CloudOperationResult.success();
    } catch (e, stack) {
      _errorHandler.handle(e, type: ErrorType.network, stackTrace: stack);
  ...
  @override
  Future<CloudOperationResult<TaskFinanceLink>> get(
    String documentId,
    String userId,
  ) async {
    try {
      ...
      return CloudOperationResult.success(
        data: TaskFinanceLink.fromFirestore(doc.id, doc.data()!),
        documentId: doc.id,
      );
    } catch (e, stack) {
      _errorHandler.handle(e, type: ErrorType.network, stackTrace: stack);
  ...
  @override
  Future<CloudOperationResult<List<TaskFinanceLink>>> getAll(
    String userId,
  ) async {
    try {
      final query =
      ...
      final items = query.docs
          .map((doc) => TaskFinanceLink.fromFirestore(doc.id, doc.data()))
          .toList();
      _logger.debug(
        'Finance',
        '[FirestoreTaskFinanceLinkStorage] Fetched ${items.length} items',
      );
  ...
  @override
  Future<CloudOperationResult<List<TaskFinanceLink>>> getModifiedSince(
    String userId,
    DateTime since,
  ) async {
    try {
  ...
  Future<CloudOperationResult<void>> batchWrite(
    ...
    try {
      ...
      _logger.debug(
        'Finance',
        '[FirestoreTaskFinanceLinkStorage] Batch write: ${items.length} items',
      );
      return CloudOperationResult.success();
    } catch (e, stack) {
      _errorHandler.handle(e, type: ErrorType.network, stackTrace: stack);
  ...
  @override
  Stream<List<TaskFinanceLink>> watchAll(String userId) {
  ...
  @override
  Stream<List<TaskFinanceLink>> watchModifiedSince(
    String userId,
    DateTime since,
  ) {
    return _collection(userId)
