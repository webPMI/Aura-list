...
class FirestoreBudgetStorage implements ICloudStorageWithTimeout<Budget> {
  static const String collectionName = 'finance_budgets';
  final ErrorHandler _errorHandler;
  final LoggerService _logger = LoggerService();

  FirestoreBudgetStorage(this._errorHandler);
  ...
  @override
  Future<CloudOperationResult<Budget>> create(
    Budget item,
    String userId,
  ) async {
  ...
  @override
  Future<CloudOperationResult<Budget>> createWithTimeout(
    Budget item,
    String userId,
    Duration timeout,
  ) async {
    try {
      ...
      _logger.debug('Finance', '[FirestoreBudgetStorage] Created: ${item.id}');
  ...
  Future<CloudOperationResult<void>> updateWithTimeout(
    ...
    try {
      ...
      _logger.debug(
        'Finance',
        '[FirestoreBudgetStorage] Updated: $documentId',
      );
  ...
  Future<CloudOperationResult<void>> delete(
    ...
    try {
      ...
      _logger.debug('Finance', '[FirestoreBudgetStorage] Deleted: $documentId');
      return CloudOperationResult.success();
    } catch (e, stack) {
      _errorHandler.handle(e, type: ErrorType.network, stackTrace: stack);
      return CloudOperationResult.failure(e.toString());
  ...
  @override
  Future<CloudOperationResult<Budget>> get(
    String documentId,
    String userId,
  ) async {
    try {
      ...
      return CloudOperationResult.success(
        data: Budget.fromFirestore(doc.id, doc.data()!),
        documentId: doc.id,
      );
    } catch (e, stack) {
      _errorHandler.handle(e, type: ErrorType.network, stackTrace: stack);
  ...
  @override
  Future<CloudOperationResult<List<Budget>>> getAll(String userId) async {
    try {
      final query =
      ...
      _logger.debug(
        'Finance',
        '[FirestoreBudgetStorage] Fetched ${items.length} items',
      );
  ...
  @override
  Future<CloudOperationResult<List<Budget>>> getModifiedSince(
    String userId,
    DateTime since,
  ) async {
    try {
  ...
  Future<CloudOperationResult<void>> batchWrite(
    ...
    try {
      ...
      _logger.debug(
        'Finance',
        '[FirestoreBudgetStorage] Batch write: ${items.length} items',
      );
      return CloudOperationResult.success();
    } catch (e, stack) {
      _errorHandler.handle(e, type: ErrorType.network, stackTrace: stack);
  ...
  @override
  Stream<List<Budget>> watchModifiedSince(String userId, DateTime since) {
    return _collection(userId)
        .where('lastUpdatedAt', isGreaterThan: since.millisecondsSinceEpoch)
        .snapshots()
        .map(
