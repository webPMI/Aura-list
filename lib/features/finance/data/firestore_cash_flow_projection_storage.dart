...
class FirestoreCashFlowProjectionStorage
    implements ICloudStorageWithTimeout<CashFlowProjection> {
  static const String collectionName = 'finance_cash_flow_projections';
  final ErrorHandler _errorHandler;
  final LoggerService _logger = LoggerService();

  FirestoreCashFlowProjectionStorage(this._errorHandler);
  ...
  @override
  Future<CloudOperationResult<CashFlowProjection>> create(
    CashFlowProjection item,
    String userId,
  ) async {
  ...
  @override
  Future<CloudOperationResult<CashFlowProjection>> createWithTimeout(
    CashFlowProjection item,
    String userId,
    Duration timeout,
  ) async {
    try {
      ...
      _logger.debug(
        'Finance',
        '[FirestoreCashFlowProjectionStorage] Created: ${item.id}',
      );
  ...
  Future<CloudOperationResult<void>> updateWithTimeout(
    ...
    try {
      ...
      _logger.debug(
        'Finance',
        '[FirestoreCashFlowProjectionStorage] Updated: $documentId',
      );
  ...
  Future<CloudOperationResult<void>> delete(
    ...
    try {
      ...
      _logger.debug(
        'Finance',
        '[FirestoreCashFlowProjectionStorage] Deleted: $documentId',
      );
      return CloudOperationResult.success();
    } catch (e, stack) {
      _errorHandler.handle(e, type: ErrorType.network, stackTrace: stack);
  ...
  @override
  Future<CloudOperationResult<CashFlowProjection>> get(
    String documentId,
    String userId,
  ) async {
    try {
      ...
      return CloudOperationResult.success(
        data: CashFlowProjection.fromFirestore(doc.id, doc.data()!),
        documentId: doc.id,
      );
    } catch (e, stack) {
      _errorHandler.handle(e, type: ErrorType.network, stackTrace: stack);
  ...
  @override
  Future<CloudOperationResult<List<CashFlowProjection>>> getAll(
    String userId,
  ) async {
    try {
      final query =
      ...
      final items = query.docs
          .map((doc) => CashFlowProjection.fromFirestore(doc.id, doc.data()))
          .toList();
      _logger.debug(
        'Finance',
        '[FirestoreCashFlowProjectionStorage] Fetched ${items.length} items',
      );
  ...
  @override
  Future<CloudOperationResult<List<CashFlowProjection>>> getModifiedSince(
    String userId,
    DateTime since,
  ) async {
    try {
  ...
  Future<CloudOperationResult<void>> batchWrite(
    ...
    try {
      ...
      _logger.debug(
        'Finance',
        '[FirestoreCashFlowProjectionStorage] Batch write: ${items.length} items',
      );
      return CloudOperationResult.success();
    } catch (e, stack) {
      _errorHandler.handle(e, type: ErrorType.network, stackTrace: stack);
  ...
  @override
  Stream<List<CashFlowProjection>> watchAll(String userId) {
    return _collection(userId)
        ...
        .map(
          (snapshot) => snapshot.docs
              .map(
                (doc) => CashFlowProjection.fromFirestore(doc.id, doc.data()),
              )
              .toList(),
        );
  }
  ...
  @override
  Stream<List<CashFlowProjection>> watchModifiedSince(
    String userId,
    DateTime since,
  ) {
    return _collection(userId)
